...
class LlamaModel:
    ...
    def _init_to_get_rotary(self):
        ...
        # Match Transformers' Llama 3 RoPE construction.  Llama 3 does not
        # split the dimensions at one arbitrary index: it divides only the
        # low-frequency bands by ``factor`` and smoothly interpolates the
        # bands between the configured wavelength cutoffs.
        dim = self.model_config.head_dim
        inv_freq = 1.0 / (
            base ** (
                torch.arange(0, dim, 2, device="cuda", dtype=torch.float32) / dim
            )
        )
        ...
        max_seq_len = int(max_position_embeddings)
